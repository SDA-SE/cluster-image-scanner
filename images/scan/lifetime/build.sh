...
descr="Clusterscan Scanner Lifetime"
...
buildah config \
  --label "${oci_prefix}.url=https://quay.io/sdase/cluster-image-scanner-scan-lifetime" \
  --label "${oci_prefix}.source=https://github.com/SDA-SE/clusterscanner-scan-lifetime" \
  --label "${oci_prefix}.revision=$( git rev-parse HEAD )" \
  --label "${oci_prefix}.version=${VERSION}" \
  --label "${oci_prefix}.title=Clusterscanner Scanner Lifetime" \
